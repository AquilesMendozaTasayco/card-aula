...
import {
  ...
  Mail, Phone, MapPin, Calendar, BookOpen, User,
  GraduationCap, Hash, Eye, ChevronDown
} from "lucide-react";
...
export default function AdminEstudiantesPage() {
  ...
  const abrirEditar = (est) => {
    setEditing(est);
    setForm({ ...CAMPO_VACIO, ...est });
    setShowModal(true);
  };

  const guardar = async () => {
    if (!form.nombres || !form.apellidos || !form.email) {
      Swal.fire({ icon: "warning", title: "Completa los campos obligatorios", text: "Nombres, apellidos y email son requeridos.", confirmButtonColor: COLOR.rojo });
      return;
    }
    try {
      if (editing) {
        await updateDoc(doc(db, "estudiantes", editing.id), { ...form, updatedAt: serverTimestamp() });
      } else {
        // Crear también usuario en colección "usuarios" para que pueda hacer login
        const ref = await addDoc(collection(db, "estudiantes"), { ...form, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
        // Nota: el uid real se asocia cuando el estudiante haga su primer login
        // o cuando se cree desde Firebase Auth por separado
      }
      setShowModal(false);
      fetchEstudiantes();
      Swal.fire({ icon: "success", title: editing ? "Actualizado" : "Estudiante registrado", timer: 1500, showConfirmButton: false });
    } catch {
  ...
  const Input = ({ field, type = "text", placeholder }) => (
    <input
      ...
      className="w-full border-b-2 border-stone-200 bg-transparent py-2 text-sm font-medium text-slate-900 outline-none transition-all focus:border-red-400 placeholder:text-stone-300"
    />
  );
      ...
      <div className="mx-auto max-w-7xl">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-40">
            <div className="h-10 w-10 animate-spin rounded-full border-4 border-t-transparent" style={{ borderColor: `${COLOR.rojo} transparent ${COLOR.rojo} ${COLOR.rojo}` }} />
          </div>
        ) : filtrados.length === 0 ? (
          ...
          <div className="bg-white border border-stone-200 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-stone-100 bg-stone-50">
              {["#", "Estudiante", "DNI / ID", "Contacto", "Ciudad", "Estado", "Acciones"].map((h, i) => (
            ...
            {filtrados.map((est, idx) => (
              ...
              >
                <div className="col-span-1 text-[11px] font-black text-stone-300">
                ...
                <div className="col-span-1">
                  <span className={`inline-block px-2 py-1 text-[9px] font-black uppercase tracking-tighter rounded-full ${
                    ...
                    est.estado === "Suspendido" ? "bg-orange-50 text-orange-600" :
                ...
                <div className="col-span-2 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setVistaDetalle(est)} className="p-2 hover:bg-stone-100 text-stone-400 hover:text-slate-700 transition-colors rounded">
                    <Eye size={14} />
                  </button>
                  <button onClick={() => abrirEditar(est)} className="p-2 hover:bg-stone-100 text-stone-400 hover:text-slate-700 transition-colors rounded">
                    <Pencil size={14} />
                  </button>
                  <button onClick={() => eliminar(est)} className="p-2 hover:bg-red-50 text-stone-400 hover:text-red-500 transition-colors rounded">
                    <Trash2 size={14} />
                  </button>
                </div>
              </motion.div>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-stone-100">
                  ...
                  <button onClick={() => setShowModal(false)} className="text-stone-400 hover:text-slate-900 transition-colors">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="p-8 space-y-8">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <Field label="Correo Electrónico" icon={Mail} required><Input field="email" type="email" placeholder="estudiante@correo.com" /></Field>
                      <Field label="Teléfono" icon={Phone}><Input field="telefono" placeholder="Ej: +51 999 888 777" /></Field>
                ...
                <div className="flex border-t border-stone-100">
                  ...
                  <button onClick={guardar}
                    className="flex-[2] px-8 py-5 text-[10px] font-black uppercase tracking-[0.3em] text-white transition-all flex items-center justify-center gap-2"
                    style={{ backgroundColor: COLOR.rojo }}>
                    <Save size={15} />
                ...
                <div className="p-8">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={label} className="flex items-center gap-3 py-2 border-b border-stone-50">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-wider text-stone-400 w-20">{label}</span>
                  ...
                  <div className="flex gap-2 mt-6">
                    ...
                    <button onClick={() => { setVistaDetalle(null); eliminar(vistaDetalle); }}
                      className="py-3 px-4 text-[10px] font-black uppercase tracking-wider text-red-500 border border-red-200 hover:bg-red-50 transition-colors">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
